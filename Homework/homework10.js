...
const calculateTotalPrice1 = items => {
    ...
    for (let fruit in items) {
        total += prices[fruit] * items[fruit]
    }
    return total
}
...
/* Task02
Write a function named calculateTotalPrice2() which takes an object of some shopping items with their quantities
...
const calculateTotalPrice2 = items => {
    ...
    for(let fruit in items){
        const noSale = items[fruit] * prices[fruit];
        let sale = 0;
        if(fruit === 'Apple') sale = (Math.floor(items[fruit] / 2) * prices[fruit]) / 2
        else if(fruit === 'Mango') sale = (Math.floor(items[fruit] / 4) * prices[fruit])
...
const isArmstrong = num => {
    return num.toFixed().split('').reduce((total, i) => total + Math.pow(i, num.toFixed().length), 0) === num
}
